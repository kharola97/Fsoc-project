...
module.exports.createRecipe = async function (req, res) {
  try {
    ...
    if (!isValidName(dishname))
      return res
        .status(400)
        .send({ status: false, message: "Dish name shoulb be a string type" });
      ...
      for (let i = 0; i < ingredients.length; i++) {
        if (!isValidString(ingredients[i])) {
          return res
            ...
            .send({
              status: false,
              message: "Ingredients should be a string type",
            });
        }
      }
    }
      ...
      for (let i = 0; i < instructions.length; i++) {
        if (!isValidData(instructions[i])) {
          return res
            ...
            .send({
              status: false,
              message: "instructions should be a string type",
            });
        }
      }
    ...
    if (!isValidTime(cookingtime))
      return res
        ...
        .send({
          status: false,
          message: "cookingtime shoulb be a number in minutes",
        });
    if (!isValidData(description))
      return res
        ...
        .send({
          status: false,
          message: "description shoulb be a string type",
        });
    ...
    if (!isValidRating(rating))
      return res
        .status(400)
        .send({ status: false, message: "Rating can only be from 0-5" });

    let finalRecipe = await createRecipe(data);
    return res
      .status(201)
      .send({ status: true, message: "Success", data: finalRecipe });
  } catch (error) {
  ...
  try {
    ...
    if (!result) {
      return res
        ...
        .send({
          status: false,
          message: "No products found with filters that have been applied",
        });
    } else {
      return res
        .status(200)
        .send({ status: true, message: "successfull", data: result });
    }
  } catch (error) {
    res.status(500).send({ status: false, message: error.message });
  }
...
module.exports.updateRecipe = async (req, res) => {
  try {
    ...
    if (!recipeToUpdate)
      return res
        .status(400)
        .send({ status: false, message: "No recipe found" });
    ...
    if (Object.keys(data).length == 0) {
      return res
        .status(400)
        .send({ status: false, message: "Enter the fields to Update" });
    }
    if (dishname) {
      if (!isValidName(dishname))
        return res
          ...
          .send({
            status: false,
            message: "Dish name shoulb be a string type",
          });
    ...
    return res
      .status(201)
      .send({ status: true, message: "Success", data: updatedRecipe });
  } catch (error) {
    res.status(500).send({ status: false, message: error.message });
  }
};
