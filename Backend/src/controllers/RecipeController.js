...
module.exports.createRecipe = async function (req, res) {
  ...
  const { dishname, cookingtime, ingredients, description, commentsId, instructions, rating } = data
  if (Object.keys(data).length == 0 || !dishname || !description || !ingredients || !cookingtime || !instructions || !rating) {
    return res
      .status(400)
